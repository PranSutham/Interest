...
export default class Loan {
    ...
    constructor(name, lender, type, fedType, gradDay, gradMonth, gradYear, disbDay, disbMonth, disbYear, prin, currBal, loanLength, yearRate, grace, graceUse, salary) {
        // Inputs
        ...
        this.gradDate = new Date(this.gradYear, this.gradMonth, this.gradDay); // Intmd 2 aka Output 1
        this.tempGraceDate = new Date(this.gradYear, this.gradMonth, this.gradDay); // Intmd 3
        this.tempDisbDate = new Date(this.gradYear, this.gradMonth, this.gradDay); // Intmd 4 <-- doesn't really matter what I set it to here :P 


        this.remDaysToGrad = readInt((this.gradDate.getTime() - currentDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    correctLoanVariables() // call this function once after a Loan object has been created in order to change the variables in this function to their correct values
    {
        ...
        this.tempGraceDate.setMonth(this.tempGraceDate.getMonth() + this.graceUse);

        this.remDaysToGrace = (this.tempGraceDate.getTime() - currentDate.getTime()) / (1000 * 60 * 60 * 24);

        this.tempGraceDate.setMonth(this.tempGraceDate.getMonth() - this.graceUse); // Since other loans will depend on the temp grace date being equal to the grad date, I must change it back to the default


        // Correcting Intmd 7

        this.tempDisbDate.setDate(this.disbDay);
        this.tempDisbDate.setMonth(this.disbMonth);
        this.tempDisbDate.setFullYear(this.disbYear);

        this.daysFromDisbtoGrad = (this.gradDate.getTime() - this.tempDisbDate.getTime()) / (1000 * 60 * 60 * 24);
        ...
        this.daysFromDisbtoGrace = (this.tempGraceDate.getTime() - this.tempDisbDate.getTime()) / (1000 * 60 * 60 * 24);
        ...
        if (this.fedType == "N/A") {
            ...
            if (this.graceUse > 0) {
                ...
                this.startBal = this.currBal + (this.prin * this.remDaysToGrad * this.dayRate);
            }
        }
        ...
        this.loanSafetyRatio = this.startBal / this.salary * 100; // returns percentage number without the sign
...
// var this.gradDate = new Date(this.gradYear, this.gradMonth, this.gradDay); // Intmd 2 aka Output 1
// var this.tempGraceDate = new Date(this.gradYear, this.gradMonth, this.gradDay); // Intmd 3
// var this.tempDisbDate = new Date(this.gradYear, this.gradMonth, this.gradDay); // Intmd 4 <-- doesn't really matter what I set it to here :P
